...
export default function Home() {
  ...
  const loadGroups = async (projectId: string) => {
    setLoading(true)
    try {
      const { data, error } = await supabase.from("groups").select("*").eq("project_id", projectId)
  ...
  const loadAllData = async () => {
    try {
      ...
      const { data: endpointsData, error: endpointsError } = await supabase.from("endpoints").select(`
          *,
          groups (
            id,
            name,
  ...
  const addEndpoint = async () => {
    console.log("[v0] Starting addEndpoint with:", { newEndpoint, currentProject })
    ...
    try {
      ...
      console.log("[v0] Group search result:", { existingGroup, groupFindError })
      ...
      if (existingGroup) {
        ...
        console.log("[v0] New group creation result:", { newGroup, groupError })
      ...
      setNewEndpoint({ name: "", path: "", method: "GET", group: "General", description: "" })
  ...
  const updateResponse = async () => {
    ...
    try {
      ...
      const updatedEndpoints = endpoints.map((ep) =>
        ep.id === selectedEndpoint.id ? { ...ep, responses: updatedResponses } : ep,
      )
  ...
  const updateFieldDescriptions = async () => {
    ...
    try {
      ...
      const updatedEndpoints = endpoints.map((ep) =>
        ep.id === selectedEndpoint.id ? { ...ep, fieldDescriptions: updatedFieldDescriptions } : ep,
      )
  ...
  const updatePath = async () => {
    ...
    setLoading(true)
    try {
      const formattedPath = editingPath.startsWith("/") ? editingPath : `/${editingPath}`
      ...
      const updatedEndpoints = endpoints.map((ep) =>
        ep.id === selectedEndpoint.id ? { ...ep, name: editingName, description: editingDescription, path: formattedPath, method: editingMethod } : ep,
      )
  ...
  const updateGroupName = async () => {
    ...
    try {
      ...
      if (selectedEndpoint && selectedEndpoint.group === groups.find(g => g.id === editingGroupId)?.name) {
        setSelectedEndpoint({
          ...selectedEndpoint,
  ...
  const updateNameDescription = async () => {
    ...
    try {
      ...
      const updatedEndpoints = endpoints.map((ep) =>
        ep.id === selectedEndpoint.id ? { ...ep, name: editingName, description: editingDescription } : ep,
      )
  ...
  const extractFieldsFromResponse = (jsonString: string): FieldDescription[] => {
    try {
      ...
      <div className="min-h-screen bg-black text-white">
          ...
          <div className="flex items-center justify-between px-6 py-4">
            <div className="flex items-center space-x-6">
              ...
              <nav className="flex space-x-6">
                <button onClick={() => setView("projects")} className="text-gray-300 hover:text-white">
                  Projects
                </button>
        ...
        <main className="p-8">
          <div className="mb-8">
            ...
            <p className="text-gray-400">Manage your mock API projects and endpoints</p>
          </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="bg-gray-800 text-blue-400 px-2 py-1 rounded font-mono text-xs">
                      {allGroups.filter((group) => group.project_id === project.id).length}
                    </span>
                  </div>
                  ...
                  {currentProject?.id === project.id && (
                    <div className="flex items-center space-x-2 pt-2">
                      ...
                      <span className="text-green-400 text-xs font-medium">Current Project</span>
                    </div>
                  )}
                </div>
    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <div className="flex items-center space-x-6">
            ...
            <nav className="flex space-x-6">
              ...
              <button className="text-gray-300 hover:text-white border-b-2 border-pink-500 pb-1">API Reference</button>
            </nav>
          </div>
      ...
      <div className="flex">
        <aside className="w-80 bg-gray-950 border-r border-gray-800 h-screen overflow-y-auto">
          <div className="p-4">
            <h2 className="text-sm font-semibold text-gray-400 mb-4">ENDPOINTS</h2>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="whitespace-normal break-words break-all leading-relaxed">
                        {groupName}  <span className="text-xs text-gray-500">({groupEndpoints.length})</span>
                      </div>
                  ...
                  <div className="space-y-1 ml-4">
                    {groupEndpoints.map((endpoint) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-md text-sm flex items-center justify-between group ${selectedEndpoint?.id === endpoint.id
                      ...
                      >
                        <div className="flex items-center overflow-hidden">
                          <span
                            className={`px-2 py-1 rounded text-xs font-medium mr-2 ${endpoint.method === "GET"
          ...
          <div className="flex-1 p-8">
            {selectedEndpoint ? (
              <div>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setEditingDescription(selectedEndpoint.description || "")
                ...
                <div className="bg-gray-900 rounded-lg p-4 mb-6">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      <span
                        className={`px-3 py-1 rounded text-sm font-medium ${getMethodColor(selectedEndpoint.method)}`}
                      >
                        {selectedEndpoint.method}
                      </span>
                      <code className="text-pink-400 font-mono">{selectedEndpoint.path}</code>
                    </div>
                    <button
                      onClick={() => {
                ...
                <div className="mb-6">
                  ...
                  <div className="flex space-x-2 mb-4">
                    {Object.keys(selectedEndpoint.responses)
                      ...
                      .map((status) => (
                        <button
                          key={status}
                          onClick={() => setSelectedStatus(Number.parseInt(status))}
                          className={`px-3 py-1 rounded text-sm font-medium ${selectedStatus === Number.parseInt(status)
                  ...
                  <div className="bg-gray-900 rounded-lg p-4 mb-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-gray-400">Status {selectedStatus} Response</span>
                      <div className="flex items-center space-x-2">
                        <button
                        ...
                        <button
                          onClick={() => {
                            setEditingResponse(selectedEndpoint.responses[selectedStatus] || "")
                    ...
                    <pre className="text-sm text-gray-300 overflow-x-auto">
                      <code>{selectedEndpoint.responses[selectedStatus] || "No response defined"}</code>
                    </pre>
                  </div>
                </div>
                ...
                {selectedEndpoint.fieldDescriptions?.[selectedStatus] && (
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-xl font-semibold text-white">Field Descriptions</h2>
                      <button
                        onClick={() => {
                          setEditingFieldDescriptions(selectedEndpoint.fieldDescriptions?.[selectedStatus] || [])
                          setShowFieldDescriptions(true)
                        }}
                        className="text-gray-400 hover:text-white flex items-center space-x-1"
                      >
                    ...
                    <div className="bg-gray-900 rounded-lg p-4">
                      <div className="space-y-3">
                        {selectedEndpoint.fieldDescriptions[selectedStatus].map((field, index) => (
                          <div key={index} className="border-b border-gray-800 pb-3 last:border-b-0">
                            <div className="flex items-center space-x-3 mb-1">
                              <code className="text-pink-400 font-mono text-sm">{field.key}</code>
                              <span
                                className={`px-2 py-1 rounded text-xs font-medium ${field.type === "STRING"
                                  ? "bg-green-600 text-white"
              ...
              <div className="text-center py-12">
                <h2 className="text-2xl font-semibold text-gray-400 mb-4">Select an endpoint</h2>
                <p className="text-gray-500">Choose an endpoint from the sidebar to view its documentation</p>
              </div>
            )}
          </div>
        </main>
      ...
      {showEditResponse && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-4xl max-h-[80vh] overflow-y-auto">
            <h3 className="text-lg font-semibold text-white mb-4">Edit Response for Status {selectedStatus}</h3>

            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-300 mb-2">Response JSON</label>
              <textarea
      ...
      {showFieldDescriptions && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-4xl max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-white">Field Descriptions for Status {selectedStatus}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <div className="flex items-end">
                        <button
                          onClick={() => {
                            const updated = editingFieldDescriptions.filter((_, i) => i !== index)
                            setEditingFieldDescriptions(updated)
                          }}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-white">Add New Endpoint</h3>
      ...
      {showEditPath && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-white">Edit Endpoint</h3>
              <button onClick={() => setShowEditPath(false)} className="text-gray-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
      ...
      {showEditGroupName && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-white">Edit Group Name</h3>
              <button onClick={() => setShowEditGroupName(false)} className="text-gray-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
      ...
      {showEditNameDescription && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-white">Edit Name & Description</h3>
              <button onClick={() => setShowEditNameDescription(false)} className="text-gray-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
